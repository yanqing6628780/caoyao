...
class Backend extends CI_Controller
{
	...
	{		
		...
		// Load view
		$this->load->view('dx_backend/roles', $data);
	}
	...
	function custom_permissions()
	{
		...
		if ($this->input->post('save'))
		{
			...
			$permission_data['operater'] = $this->input->post('operater');
			$permission_data['verify'] = $this->input->post('verify');
			$permission_data['adduser'] = $this->input->post('adduser');
		...
		$data['operater'] = $this->permissions->get_permission_value($role_id, 'operater');
		$data['verify'] = $this->permissions->get_permission_value($role_id, 'verify');
		$data['adduser'] = $this->permissions->get_permission_value($role_id, 'adduser');
	
		// Load view
        $data['title'] = '自定义权限管理';
		$this->load->view('dx_backend/custom_permissions', $data);
	}
    
    function users_regions()
    {
        $this->load->model('dx_auth/users', 'users');
        $this->load->model('shopex/region_mdl', 'region_mdl');        
		
        $user_id = $this->input->post('user_id') ? $this->input->post('user_id') : 1;
        
		// Get all users
		$data['users'] = $this->users->get_all()->result();        
        $data['region_arr'] = getRegionArray($user_id);  
        
        if ($this->input->post('save')){
            $regions = $this->input->post('action_code');
            $region_ids = join(',', $regions);
            $all = $this->input->post('all');
            
            $regions = $this->input->post('action_code');    
            
            $this->ur_mdl->setUserId($user_id);
            $isExist = $this->ur_mdl->checkUserId();
            
            if($all){
                $array['region_ids'] = 'all';
            }else{
                $array['region_ids'] = $region_ids;
            }
            if($isExist){            
                $this->ur_mdl->setData($array);
                $this->ur_mdl->update();
                $data['msg'] = '更新成功';
                $data['success'] = true;
            }else{
		...
		$this->load->view('backend/users_regions');
