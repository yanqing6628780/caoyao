...
class Auth extends CI_Controller
{
	...
	function username_check($username)
	{
		...
		if ( ! $result)
		{
			$this->form_validation->set_message('username_check', '用户名已存在。请重新填写用户名');
		}
	...
	{
		$result = TRUE;
		$code = strtoupper($code);//转换为大写
        
		if ($this->dx_auth->is_captcha_expired())
		{
			// Will replace this error msg with $lang
			$this->form_validation->set_message('captcha_check', '您的验证码已过期。请再试一次。');			
			$result = FALSE;
		}
		elseif ( ! $this->dx_auth->is_captcha_match($code))
		{
			$this->form_validation->set_message('captcha_check', '您的验证码与图像不匹配。请再试一次。');			
			$result = FALSE;
		}
	...
	function recaptcha_check()
	{
		...
		if ( ! $result)
		{
			$this->form_validation->set_message('recaptcha_check', '您的验证码与图像不匹配。请再试一次。');
		}
	...
	{		
		...
		{	
			...
			// Run form validation and register user if it's pass the validation
			if ($val->run() AND $this->dx_auth->register($val->set_value('username'), $val->set_value('password'), ''))
			{				
				// Load registration success page
				redirect(site_url('/backend/unactivated_users/'));
			}
			else
			{
