...
class Users extends CI_Model 
{
	...
	function get_all($offset = 0, $row_count = 0, $where = array())
	{
		...
		if ($offset >= 0 AND $row_count > 0)
		{
			...
			$this->db->select("$roles_table.name AS role_name, $roles_table.cnname as role_cnname", FALSE);
    ...
    function get_fields($fields, $InRoleId = array())
	{
		$this->db->select($fields);
        if($InRoleId){
            $this->db->where_in('role_id', $InRoleId);
        }
		return $this->db->get($this->_table);
	}
}
