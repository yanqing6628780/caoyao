...
class DX_Auth
{
	...
	*/
    ...
    // Get user cnname string
	function get_user_cnname()
	{
		return $this->ci->session->userdata('DX_user_cnname');
	}
    ...
    **当不需要邮件验证时,能添加用户角色
    */
	function register($username, $password, $email, $role_id = 1)
	{		
		// Load Models
		$this->ci->load->model('dx_auth/users', 'users');
		$this->ci->load->model('dx_auth/user_temp', 'user_temp');
		...
		{				
            ...
            //pfofile_id
            $new_user['pfofile_id'] = $this->ci->db->insert_id();
		}
