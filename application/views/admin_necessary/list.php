            ...
            <div class="portlet-body">
                <table class='table table-striped table-bordered table-hover'>
                    ...
                    <?php foreach($result as $key => $row):?>
                        <tr>
                            <td><?=$row['necessities_scheme_name']?></td>
                            <td>
                                <?php if (checkPermission2('necessary_edit')): ?>
                                <a  onclick="necessaries_edit(<?=$row['id']?>)" class="btn green"> <i class=" icon-list-ol icon-white"></i> 必需品表</a>
...
function del(id, table){
    common_del('<?=site_url($controller_url."del")?>', id, table, "#necessary_view");
}
function necessaries_edit(id){
    LoadPageContentBody('<?=site_url($controller_url."necessaries_edit")?>', {id: id})
}
